...
class Solution {
  public:
    ...
    int solve2(vector<int> &arr, int k){
        //TC=O(k*logn)
        priority_queue<int,vector<int>,greater<int>>pq(arr.begin(),arr.end());//Due to Heapify, Heap creation in O(n)
    ...
    //Method-03: Using max heap (optimal), always maintain size of heap==k
