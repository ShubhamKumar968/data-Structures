  ...
  //Method-01: Recursion + Memoization
    ...
    int solve(vector<int> &prices, int idx, bool buy){
        ...
        if(buy){//buy=1
            ...
            int buys= -prices[idx]+ solve(prices,idx+1, !buy);//buy karne ke baad sell ka function call karenge.
            int skip= 0 +  solve(prices,idx+1,buy);
            
            profit=max(buys,skip);
        }else{ //buys=0 -> sell
        
            int sells= prices[idx]+ solve(prices,idx+2, !buy);//sell karne ke baad buy ka function call karenge.
            int skip= 0 +  solve(prices,idx+1,buy);
        ...
        for (int idx = n - 1; idx >= 0; idx--) {
            for (int buy = 0; buy <= 1; buy++) {
                ...
                if (buy == 1) { // We are allowed to buy
                    ...
                    int do_sell = prices[idx] + dp[idx + 2][1]; // sell and flip state to 1
